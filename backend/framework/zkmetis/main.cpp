...
void print_help(const string exeName, string errorMsg) {
    ...
    cout << YELLOW << "Usage:\n$ ";
    cout << GREEN << exeName << YELLOW << " " << zkmips_file_prefix << RESET << " <zkMIPS assembly file path> [" << YELLOW << timesteps_prefix << RESET << " <trace length log_2>] [" << YELLOW << security_prefix << RESET << " <security parameter]> [" << YELLOW << primary_tape_prefix << RESET << " <primaryTapeFile>] [" << YELLOW << private_tape_prefix << RESET << " <auxTapeFile>] [" << YELLOW << run_verifier_prefix << RESET << " | " << YELLOW << run_prover_prefix << RESET << "] [" << YELLOW << address_port_prefix << RESET << " <address:port_number>] [" << YELLOW << session_prefix << RESET << " <sessionID>]" << endl << endl;
    ...
    cout << YELLOW << show_asm_prefix   << RESET << "  : Display zkMIPS assembly input" << endl;
    cout << YELLOW << verbose_prefix    << RESET << "   : Verbose output, print BAIR, ACSP, APR and FRI specifications" << endl;
    cout << YELLOW << no_proof_prefix   << RESET << "  : Run the zkMIPS execution engine (i.e., without generating a proof)." << endl << endl;

    cout << YELLOW << zkmips_file_prefix << RESET << "       : Path to the zkMIPS assembly code " << YELLOW << "(required)" << RESET << endl;
...
void print_examples(const string exeName) {
    cout << YELLOW << "Example:\n$ " << RESET;
    cout << exeName << " examples-zkmips/simple_add.asm " << timesteps_prefix << " 10 " << security_prefix << " 120" << endl;
    ...
    cout << exeName << " ./examples-zkmips/read_test.asm " << timesteps_prefix << " 10 " << security_prefix << " 120 " << primary_tape_prefix << " ./examples-zkmips/read_test.pubtape " << private_tape_prefix <<" ./examples-zkmips/read_test.auxtape" << endl;
    cout << endl << YELLOW << "The simple read from tapes example over the network:\n$ " << RESET;
    cout << exeName << " examples-zkmips/read_test.asm " << timesteps_prefix << " 10 " << security_prefix << " 120 " << primary_tape_prefix << " ./examples-zkmips/read_test.pubtape " << private_tape_prefix <<" ./examples-zkmips/read_test.auxtape " << run_verifier_prefix << " " << address_port_prefix << "localhost:2324" << YELLOW << "\n$ " << RESET;
    cout << exeName << " examples-zkmips/read_test.asm " << timesteps_prefix << " 10 " << security_prefix << " 120 " << primary_tape_prefix << " ./examples-zkmips/read_test.pubtape " << private_tape_prefix <<" ./examples-zkmips/read_test.auxtape " << run_prover_prefix << " " << address_port_prefix << "localhost:2324" << endl;

    cout << endl << YELLOW << "Knowledge of Factorization example:\n$ " << RESET;
    cout << exeName << " examples-zkmips/knowledge_of_factorization.asm " << timesteps_prefix << " 10 " << security_prefix << " 120 " << private_tape_prefix << " ./examples-zkmips/knowledge_of_factorization_auxtape.txt" << endl;
}
...
void printHeader(void) {
    ...
    cout << endl << "   / /";
    cout << endl << "  / /_";
    cout << endl << " /____|" << RESET << endl << endl;
}
