...
bool is_zkmips_label(const string &str) { // check if a string is a label
    string prefix = "__";
    string suffix = "__";
...
map<string, int> RAMProgram::buildLabelsMap(vector<string>& lines){
    ...
    for (auto& l : lines){
        ...
        if (is_zkmips_label(splitted_line[0])) { // if label starts with prefix and ends with suffix then it's a valid label
