...
string get_immediate_or_register(const string& reg_or_imm) {
    if (reg_or_imm.empty()) {
        return reg_or_imm;
    } else if (is_zkmips_label(reg_or_imm)) {
        return reg_or_imm;
...
string zkmips_to_zkmetis_instruction(string& op, string& dst , string& src1, string& src2) {
    op = stringToUpper(op);
    string zero = "$zero";
    ...
    if (DEBUG) {
        std::cout << "zkmips src1 : " << src1 << '\n';
        std::cout << "zkmips src2 : " << src2 << '\n';
    }
    src1 = get_immediate_or_register(src1);
    src2 = get_immediate_or_register(src2);
    if (DEBUG) {
        std::cout << "zkmetis src1 : " << src1 << '\n';
        std::cout << "zkmetis src2 : " << src2 << '\n';
    }
    ...
    if (op == "AND" || op == "ANDI") {
        ...
        std::cerr << "zkMIPS parser: " << op << " was not recognized" << endl;
        exit(EXIT_FAILURE);
    }
}
...
string get_native_register(const string& r) {
    if (r == "$zero" || r == "$0") {
        ...
        std::cerr << r << " : not a zkMIPS register" << endl;
        exit(EXIT_FAILURE);
    }
}
...
string parse_zkmips(const string assemblyFile, const string primaryTapeFile, const string& macros_file, const bool show_asm) {
    ...
    for (auto& l : lines) {
        ...
        switch (tokens.size()) {
            ...
            case 2:
                dst = tokens[1];
                instr = zkmips_to_zkmetis_instruction(op, dst, src1, src2);
                break;
            case 1:
                instr = op;
