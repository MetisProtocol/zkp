...
const string macros_file = "./framework/zkmetis/src/macros.json";
...
TEST(zMIPS, factorial) {
	...
	string asm_parsed = parse_zkmips(assembly_file, public_tape, macros_file, false);
...
TEST(zMIPS, mmult) {
	...
	string asm_parsed = parse_zkmips(assembly_file, "", macros_file, false);
	execute_locally(asm_parsed, private_tape, 0, securityParameter, false, true, false);
	std::remove(asm_parsed.c_str());
	EXPECT_EQ(answer_, 0);
}
